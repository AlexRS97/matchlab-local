...
El histórico de aprendizaje se comprueba a diario mientras la aplicación esté abierta. El arranque y el cierre son manuales; con MatchLab detenido no hay actualizaciones. El reentrenamiento automático está desactivado en `config/learning.yaml` para evitar cargas inesperadas. Se solicita desde Model Lab y utiliza dos hilos por modelo. La política de siete días y 150 encuentros nuevos solo se aplica si se vuelve a habilitar `automatic_training`. Si un entrenamiento falla, se conserva la versión publicada anterior y se muestra el error.
