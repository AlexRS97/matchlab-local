...
class LearningService:
    ...
    def status(self):
        ...
        return {
            ...
            "data_health": {
                ...
                "training_recommended": report is None
                or (age is not None and age >= self.config["maximum_model_age_days"])
                or (
                    new >= self.config["minimum_new_samples"]
                    and age is not None
                    and age >= self.config["retrain_days"]
                ),
    ...
    async def update(self, force_train=False):
        ...
        try:
            ...
            report = self.report()
            due = report is None or (
                now - datetime.fromisoformat(report["created_at"])
                >= timedelta(days=self.config["retrain_days"])
                and update["stored_matches"] - report["dataset"]["historical_matches"]
            ...
            self.progress("Actualización con errores; se conserva el modelo anterior")
